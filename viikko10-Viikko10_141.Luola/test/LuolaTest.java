...
public class LuolaTest {
    ...
    @Test
    public void testaaHirviöidenLiikumattomuus() throws Throwable {
        LuolaTest.P p = new LuolaTest.P(3, 3, 1, 20, false);
        String viesti = "Testattiin Luola luola = new Luola(3,3,1,20,false); luola.run();\n";
        Thread t = new Thread(p);
        t.start();
        ...
        String prev = null;
        for (String row : ekat) {
            if(!row.matches("h\\s+\\d+\\s+\\d+"))continue;
            System.out.println(row);
            if(prev == null){
                prev = row;
                continue;
            }
            if(!prev.equals(row))fail(viesti + "Hirviöiden ei tulisi liikkua!");
            prev = row;
        }
    }

    @Test
    public void testaaHirviöidenLiikkuminen() throws Throwable {
        LuolaTest.P p = new LuolaTest.P(3, 3, 1, 20, true);
        String viesti = "Testattiin Luola luola = new Luola(3,3,1,20,true); luola.run();\n";
        Thread t = new Thread(p);
        t.start();
        ...
        fail(viesti + "Hirviöiden pitäisi liikkua!");
    }

    /*
     * TODO: ei saa päästää ArrayOutOfBoundsia ulos - kts http://tmc.mooc.fi/mooc/submissions/419016
     * Revertataan ja korjataan tulevaisuudessa
	@Test
    public void testaaHirvioidenLiikkuminen() throws Throwable {
        LuolaTest.P p = new LuolaTest.P(4, 4, 15, 100, true);
        String viesti = "Testattiin Luola luola = new Luola(4,4,15,100,true); luola.run();\n";
        String viesti2 = "Tulosteesi oli:\n";
        Thread t = new Thread(p);
        t.start();
        ...
        for (int i = 2; i < 18; i++) {
            String r = ekat[i].substring(2);
            assertFalse(viesti + "\nVarmista että pelilaudalla on aluksi oikea määrä hirviöitä.", r.isEmpty());
            assertFalse(viesti + "\nVarmista että hirviöt eivät voi mennä aluksi päällekäin\n\n" + viesti2 + eka, ht.contains(r));
            ht.add(r);
        }
        for (int k = 0; k < 10; k++) {
            p.write("w\n");
            ...
            for (int i = 2; i < 18; i++) {
                ...
                assertFalse(viesti + "\nVarmista että hirviöt eivät voi liikkua toistensa päälle.\n\n" + viesti2 + eka, ht.contains(r));
                ht.add(r);
            }
        }
    }
    ...
    @Test
    public void testaaLuolanTulostus() throws Throwable {
        LuolaTest.P p = new LuolaTest.P(10, 10, 12, 100, true);
        String viesti = "Testattiin Luola luola = new Luola(10,10,12,100,true); luola.run();\n";
        String viesti2 = "Tulosteesi oli:\n";
        Thread t = new Thread(p);
        t.start();
        ...
        for (int i = 3; i < 15; i++) {
            ...
            String c = r.substring(0,1);

            int x = Integer.parseInt(r.substring(2, 3));
            int y = Integer.parseInt(r.substring(4));

            assertEquals(viesti + "\nVarmista että luola tulostuu oikein.\n\n" + viesti2 + eka + "\n\nSijainnissa X: " + x + ", Y: " + y,c,ekat[mapBase + y].substring(x,x+1));
        }
    }

    @Test
    public void testaaLuolanTulostusLiikkumisenJalkeen() throws Throwable {
        LuolaTest.P p = new LuolaTest.P(10, 10, 12, 100, true);
        String viesti = "Testattiin Luola luola = new Luola(10,10,12,100,true); luola.run();\n";
        String viesti2 = "Tulosteesi oli:\n";
        Thread t = new Thread(p);
        ...
        for (int i = 3; i < 15; i++) {
            ...
            assertEquals(viesti + "\nVarmista että luola tulostuu oikein myös liikkumisen jälkeen.\n\n" + viesti2 + eka + "\n\nSijainnissa X: " + x + ", Y: " + y,c,ekat[mapBase + y].substring(x,x+1));

        }
    }

    @Test
    public void testaaLuolanTulostusSyomisenJalkeen() throws Throwable {
        LuolaTest.P p = new LuolaTest.P(3, 1, 2, 100, false);
        String viesti = "Testattiin Luola luola = new Luola(3,1,2,100,false); luola.run();\n";
        String viesti2 = "Tulosteesi oli:\n";
        Thread t = new Thread(p);
        t.start();
        p.read();
        ...
        assertEquals(viesti + "\nVarmista että luola tulostuu oikein myös hirviön tuhoutumisen jälkeen.\n\n" + viesti2 + eka,".@h", ekat[ekat.length - 1]);
