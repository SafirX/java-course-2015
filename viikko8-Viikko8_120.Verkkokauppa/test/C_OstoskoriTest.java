...
public class C_OstoskoriTest {
    ...
    public void onMapTaiList() {
        ...
        for (Field field : kentat) {
            assertFalse(k
                    + "eli muuta " + kentta(field.toString()) + " oikean tyyppiseksi", field.toString().matches(".* java\\.util\\.[A-Z][a-z]+(Map|List) .*"));

            assertFalse(k + " muita oliomuuttujia et tarvitse, poista " + kentta(field.toString()), !field.toString().matches(".* java\\.util\\.([A-Z][a-z]+)?(Map|List) .*"));
