    ...
    public void onMap() {
        ...
        for (Field field : kentat) {
            assertFalse("Talleta " + klassName + ":n tavaroihen hinnat oliomuuttujaan Map<String, Integer> hinnat; \n"
                    + "eli muuta " + kentta(field.toString()) + " oikean tyyppiseksi", field.toString().matches(".* java\\.util\\.[A-Z][a-z]+Map .*"));
            assertFalse("Et tarvitse luokalle " + klassName + " listatyyppistä oliomuuttujaa!, poista " + kentta(field.toString()), field.toString().matches(".* java\\\\.util\\\\.([A-Z][a-z]+)*List .*"));
